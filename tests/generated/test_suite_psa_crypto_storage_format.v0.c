 ...
 *      Test suite file     : tests/suites/test_suite_psa_crypto_storage_format.function
 *      Test suite data     : tests/suites/test_suite_psa_crypto_storage_format.v0.data
 *
 */
...
static int can_exercise(const psa_key_attributes_t *attributes)
{
    ...
    /* Ed25519ph and Ed448ph work with keys of matching size only. */
    if ((alg == PSA_ALG_ED25519PH && attributes->bits != 255) ||
        (alg == PSA_ALG_ED448PH && attributes->bits != 448)) { /* !!OM-PCI-28 */
        return 0;
    }
    ...
    if (PSA_ALG_IS_JPAKE(alg)) {
        /* psa_exercise_key() does not support JPAKE at this time. */
        return 0;
    }
...
/* We currently only support one specific primitive for JPAKE.
 * https://github.com/Mbed-TLS/TF-PSA-Crypto/issues/503
 */
#  if defined(PSA_WANT_KEY_TYPE_ECC_KEY_PAIR_IMPORT) && \
    defined(PSA_WANT_ECC_SECP_R1_256)
#    define CAN_EXERCISE_JPAKE
#  endif

static int exercise_pake_key(mbedtls_svc_key_id_t key_id,
                             psa_algorithm_t policy_alg)
{
    psa_pake_operation_t client = PSA_PAKE_OPERATION_INIT;
    const uint8_t client_name[6] MBEDTLS_ATTRIBUTE_UNTERMINATED_STRING = "client";
    const uint8_t server_name[6] MBEDTLS_ATTRIBUTE_UNTERMINATED_STRING = "server";
    uint8_t output[PSA_PAKE_OUTPUT_MAX_SIZE];
    size_t output_size = sizeof(output);
    size_t output_len;
    int ok = 0;

    /* At the time of writing, psa_exercise_key() does not yet support JPAKE.
     * To get things going quickly, perform a basic check here.
     *
     * We use one particular cipher suite (picking the
     * hash algorithm from the key policy), so test cases must depend on
     * CAN_EXERCISE_JPAKE or else setup will fail.
     */

    psa_algorithm_t setup_alg = policy_alg;
    if (policy_alg == PSA_ALG_JPAKE_BASE) {
        setup_alg = PSA_ALG_JPAKE(PSA_ALG_SHA_256);
    }
    ...
    PSA_ASSERT(psa_pake_setup(&client, key_id, &cipher_suite));
    PSA_ASSERT(psa_pake_set_user(&client, client_name, sizeof(client_name)));
    PSA_ASSERT(psa_pake_set_peer(&client, server_name, sizeof(server_name)));

    /* Output the first public key and the first ZKP. Outputting a public key
     * guarantees that the primitive set in the cipher suite has been
     * exercised. Outputting a ZKP guarantees that the hash set in the cipher
     * suite has been exercised. */
                         ...
                         int flags)
{
    ...
    if ((flags & TEST_FLAG_EXERCISE) != 0) {
        if (can_exercise(&actual_attributes)) {
...
#if defined(PSA_WANT_ALG_JPAKE)
        if (PSA_ALG_IS_JPAKE(psa_get_key_algorithm(expected_attributes))) {
            TEST_ASSERT(exercise_pake_key(
                            key_id,
                      ...
                      data_t *representation, int flags)
{
    ...
    if(0) {
#ifndef PSA_WANT_RSA_KEY_SIZE_1536
    } else if (PSA_KEY_TYPE_IS_RSA(type) && bits == 1536) {
        /* !!OM-PCI-15 1536 bit RSA keys may be switched off */
#endif
#ifndef PSA_WANT_AES_KEY_SIZE_128
    } else if (type == PSA_KEY_TYPE_AES && bits == 128) {
        /* !!OM-PCI-17 128 bit AES keys may be switched off */
#endif
#ifndef PSA_WANT_AES_KEY_SIZE_192
    } else if (type == PSA_KEY_TYPE_AES && bits == 192) {
        /* !!OM-PCI-17 192 bit AES keys may be switched off */
#endif
#ifndef PSA_WANT_AES_KEY_SIZE_256
    } else if (type == PSA_KEY_TYPE_AES && bits == 256) {
        /* !!OM-PCI-17 256 bit AES keys may be switched off */
#endif
    } else
...
static int get_expression(int32_t exp_id, intmax_t *out_value)
{
    ...
    switch (exp_id) {
        ...
        case 222:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_MD5);
            }
            break;
        case 223:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_RIPEMD160);
            }
            break;
        case 224:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_SHA3_224);
            }
            break;
        case 225:
            {
        ...
        case 226:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_SHA3_384);
            }
            break;
        case 227:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_SHA3_512);
            }
            break;
        case 228:
            {
        ...
        case 229:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_SHA_224);
            }
            break;
        case 230:
            {
        ...
        case 231:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_SHA_384);
            }
            break;
        case 232:
            {
                *out_value = PSA_ALG_JPAKE(PSA_ALG_SHA_512);
            }
            break;
        case 233:
            {

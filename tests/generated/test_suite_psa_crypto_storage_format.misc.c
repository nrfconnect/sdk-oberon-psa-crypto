 ...
 *      Test suite data     : tests/suites/test_suite_psa_crypto_storage_format.misc.data
 *
 */
                         ...
                         int flags)
{
    ...
    if (flags & TEST_FLAG_OVERSIZED_KEY) {
        psa_status_t status = psa_get_key_attributes(key_id, &actual_attributes);
        if (status != PSA_ERROR_INSUFFICIENT_MEMORY) {  /* !!OM */
            TEST_EQUAL(status, PSA_ERROR_DATA_INVALID);
        }
        ok = 1;
        goto exit;
    }

...
They are located in `drivers/oberon/oberon_helpers.h`.
...
configurations located in subdirectory `config_examples/` (see
...
`include/psa/`, and a default _dispatch logic_ implementation located in
`library/`.

## Provide Crypto Driver Configuration

The _crypto driver configuration_ file provided in `targets/acme/demo/` must
...
## Adapt the Dispatch Logic

To make the set of available _hardware drivers_ known to _Oberon PSA Crypto_
and its configuration mechanism, the _dispatch logic_ C file provided in
`targets/acme/demo/` must be adapted and placed in:
...
The following header files provided in `targets/acme/demo/` may need to be
...
When using another _dispatch logic_ implementation than the one provided with

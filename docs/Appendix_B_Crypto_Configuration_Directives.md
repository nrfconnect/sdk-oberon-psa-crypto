...
elimination is described. This includes directives to be set by the
_system crypto configurator_ (`PSA_WANT_XXX`, `PSA_USE_XXX`) and internal
directives that are derived from the former.

C define directives of the form `PSA_WANT_XXX` and `PSA_USE_XXX` are defined in
_system crypto configurations_, see
[Crypto Configuration](Crypto_Configuration.md).

C define directives of the form `PSA_NEED_XXX` and `PSA_ACCEL_XXX` are defined
within the _crypto configurations_ of _hardware drivers_ and _Oberon drivers_,
see

...
3. Create a `psa/crypto_config.h` file based on, e.g., the configuration example
`crypto_config_demo.h`, and adapt it based on your existing configuration files 
`psa/crypto_config.h` and, if you are migrating from _Mbed TLS 3.x_, also on 
`mbedtls/mbedtls_config.h`. In _TF-PSA-Crypto_ and _Oberon PSA Crypto_ since 2.0, 
configuration aspects from both files are combined into `psa/crypto_config.h`.

4. Make sure that your new `psa/crypto_config.h` file defines the "wanted"
crypto features as described above.

5. In the `psa/crypto_config.h` file, add the use directives for a DRGB 
...
6. Copy the driver configuration files 
...
drivers are located in subfolders `demo/include` and `demo/library`. Make sure
to remove the demo hardware and opaque drivers for production use. Optionally,
add your own hardware drivers by adapting the driver configuration files and
the wrapper implementation. 

7. _Oberon PSA Crypto_ provides the _TF-PSA-Crypto_ mock implementations for
...
8. Copy your `crypto_config.h` and driver configuration files to your 
`include/psa` directory of _Oberon PSA Crypto_ or make sure they can be found 
in the include path.

9. Copy your `psa_crypto_driver_wrappers.c` to the dispatch folder and add 
the path to your build.
...
See [README-SSL](../../programs/README-SSL.md) for more information.

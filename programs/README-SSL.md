...
<https://github.com/Mbed-TLS/mbedtls/releases/tag/v4.1.0> or clone _Mbed TLS_ and
check out version 4.1.0 as follows:
...
Provide the path to _ocrypto_ 4.0.0 or later via _-DOCRYPTO_ROOT=path/to/ocrypto_.

Provide the path to _Mbed TLS_ 4.1.0 via _-DMBEDTLS_ROOT=path/to/mbedtls_.
...
Note that in configuration directory `mbedtls_config_TLS1_2+3`, `ssl_server2` 
can be forced to use the TLS 1.3 protocol:
...
The migration requires projects building on _Mbed TLS_ 4.1.0.

Configuration options are still limited and there are still build dependencies
to some of the _TF-PSA-Crypto_ crypto code files even though the code is not 
used; this is expected to improve in future releases of _Mbed TLS_.
...
2. In your project build files, replace code files from `TF-PSA-Crypto` with
   the path to the equivalent files in _Oberon PSA Crypto_:
    * oberon-psa-crypto/`core/*.c`
...
3. Add the _Oberon PSA Crypto_ include paths in the following order and make
    ...
    * oberon-psa-crypto/`core`
    * oberon-psa-crypto/`dispatch`
    * oberon-psa-crypto/`platform`
    * oberon-psa-crypto/`utilities`
    * oberon-psa-crypto/`extras`

...
static psa_status_t psa_validate_ecc_key_attr(const psa_key_attributes_t *attributes)
{
    ...
    /* With algorithm we can't evaluate more fields */
    if (alg == PSA_ALG_NONE) {
        return PSA_SUCCESS;
    }
                                                     ...
                                                     size_t data_length)
{
    psa_algorithm_t alg = psa_get_key_algorithm(attributes);
    psa_key_type_t type = psa_get_key_type(attributes);
    size_t key_data_length_bits = PSA_BYTES_TO_BITS(data_length);
    ...
    switch (PSA_KEY_TYPE_ECC_GET_FAMILY(type)) {
        case PSA_ECC_FAMILY_SECP_R1:
            /* secpr1p521 will require 66 bytes therefore checking for 528 bits */
            if (key_data_length_bits != 192 && key_data_length_bits != 224 &&
                key_data_length_bits != 256 && key_data_length_bits != 384 &&
                key_data_length_bits != 528) {
                return PSA_ERROR_INVALID_ARGUMENT;
            }
            break;
        case PSA_ECC_FAMILY_SECT_K1:
            if (key_data_length_bits != 233 && key_data_length_bits != 239 &&
                key_data_length_bits != 283 && key_data_length_bits != 409 &&
                key_data_length_bits != 571) {
                return PSA_ERROR_INVALID_ARGUMENT;
            }
            break;
        case PSA_ECC_FAMILY_SECT_R1:
            if (key_data_length_bits != 233 && key_data_length_bits != 283 &&
                key_data_length_bits != 409 && key_data_length_bits != 571) {
                return PSA_ERROR_INVALID_ARGUMENT;
            }
            break;
        case PSA_ECC_FAMILY_BRAINPOOL_P_R1:
            if (key_data_length_bits != 192 && key_data_length_bits != 224 &&
                key_data_length_bits != 256 && key_data_length_bits != 320 &&
                key_data_length_bits != 384 && key_data_length_bits != 512) {
                return PSA_ERROR_INVALID_ARGUMENT;
            }
            break;
        /* 255 bits will need a data size of 32 bytes == 256 bit */
        case PSA_ECC_FAMILY_TWISTED_EDWARDS:
        case PSA_ECC_FAMILY_MONTGOMERY:
            if (key_data_length_bits != 256 && key_data_length_bits != 448) {
                return PSA_ERROR_INVALID_ARGUMENT;
            }
            break;
        default:
